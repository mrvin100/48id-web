...
export function OperatorUsersModule({ accountId }: { accountId: string }) {
  ...
  if (error) return (
    <div className="space-y-6">
      ...
      <div className="text-destructive py-8 text-center">Failed to load users: {error.message}</div>
    </div>
  )
            ...
            {isLoading ? (
                ...
                <TableCell colSpan={5} className="h-32">
                  <Empty>
                    <EmptyHeader>
                      ...
                      <EmptyDescription>No users are associated with your operator account.</EmptyDescription>
                    </EmptyHeader>
                  </Empty>
                </TableCell>
              </TableRow>
              ...
              users.map(user => (
                <TableRow key={user.id}>
                  ...
                  <TableCell>{user.name ?? `${user.firstName ?? ''} ${user.lastName ?? ''}`.trim() || '—'}</TableCell>
