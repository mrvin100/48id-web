...
/**
 * Operator API Key Panel
 *
 * Create, view, rotate, and revoke the operator's API key.
 * Raw key is shown only once after creation/rotation.
 * WEB-S4-FE-09
 */
...
export function ApiKeyPanel() {
  const { data: apiKey, isLoading } = useOperatorApiKey()
  const createMutation = useCreateApiKey()
  const rotateMutation = useRotateApiKey()
  const deleteMutation = useDeleteApiKey()

  // Holds the raw key after creation/rotation — shown once
  ...
  const isOwner = apiKey?.memberRole === 'OWNER' || !apiKey

  return (
    <div className="space-y-6">
      <PageHeader
        title="API Key"
        description="Manage your operator API key"
      />
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-4 py-12">
            ...
            <p className="text-muted-foreground text-sm">
              No API key exists for your account yet.
            </p>
            {isOwner && (
              <Button
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <dl className="grid grid-cols-2 gap-2 text-sm">
              ...
              <dd className="capitalize">{apiKey.memberRole.toLowerCase()}</dd>
            </dl>
